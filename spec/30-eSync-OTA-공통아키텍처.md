...
- **GPDM** — Component와 SW BOM의 출처 원장. 릴리스 메타데이터(버전·품번·호환성·롤백)의 **단일 진실원천(SSOT)**이다(§3.2.4).
...
- **TGU** — eSync Client(오케스트레이션)와 eSync Agent(검증·전달)를 한 노드에 둔다. eSync의 신뢰 경계는 TGU까지다.
- **대상 제어기** — eSync Agent가 UDS on CAN(또는 제어기별 채널)으로 플래시하는 대상. 능력별 변주는 §1.4.
...
1. **발행** — GPDM 릴리스 → eSync Server 조립·서명·패키징(§3·§4).
...
HSM 보유 제어기의 `binary`는 **암호화·서명된 이미지**(SE Binary)이며, eSync는 이를 통짜 페이로드로 취급한다. 이미지 내부 헤더 메타데이터는 manifest.xml과 별개로 관리하되 같은 GPDM 프로퍼티에서 공동 생성한다([32 §6](32-EPOS-30i-업데이트-사양서.md)).
