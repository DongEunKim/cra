...
| **ECU(엔진 제어기)** | 모델별(혼재) | HSM 유무 분기 | 평문/암호화(분기) | 모델별 | `0x27`/`0x29`(분기) | UDS on CAN(`0x34/36/37`) | CAN | [33](33-ECU-OTA사양서.md) |
...
- **주체·키:** eSync Server가 KMS에 서명을 요청하고, KMS가 CS Cert(코드서명 인증서) 개인키로 서명한다. 개발자·벤더는 서명 키를 갖지 않는다. 이 키는 eSync Server가 장비에 자신의 신원을 증명할 때 쓰는 eSync Server 인증서의 키와 **별개**다([10 §2.2](10-PKI-사양서.md)).
...
- 이 구간에서 오가는 인증서(디바이스 인증서·eSync Server 인증서)는 **접속 상대의 신원**만 증명한다. 내려받은 소프트웨어의 진위는 이 인증서들이 아니라 Component에 동봉된 CS Cert로 검증한다(§4). 두 인증서의 구분은 [10 §2.2](10-PKI-사양서.md)·[10 §2.6](10-PKI-사양서.md).
...
1. ClientHello.
2. 서버 인증서 제시.
3. 클라이언트가 서버 인증서 검증(해당 앵커 인증서로 체인·유효기간·SAN·EKU·OCSP).
