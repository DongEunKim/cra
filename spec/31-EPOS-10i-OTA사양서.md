...
프로비저닝부터 설치까지, **어느 주체가 어느 단계에서 무엇을 하는지**를 [그림 1]로 개괄한다. EPOS-10i는 HSM이 없어 발행에 **내부 계층(암호화·이미지 서명)이 없고**(GPDM → eSync Server 직행), 검증은 **TGU(eSync Agent)가 대행**하며, 세션은 `0x27`·복구는 Insurance Kernel이다. 프로비저닝 상세는 [20](20-프로비저닝-사양서.md), 공통 골격은 [30 §2](30-eSync-OTA-공통아키텍처.md). (HSM 보유 EPOS-30i와의 대비는 [32 §3.0](32-EPOS-30i-업데이트-사양서.md).)
