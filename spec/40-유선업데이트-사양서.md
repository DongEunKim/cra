...
| **관계** | [10 PKI 사양서](10-PKI-사양서.md) · [20 프로비저닝 사양서](20-프로비저닝-사양서.md) · [30 eSync OTA 공통 아키텍처](30-eSync-OTA-공통아키텍처.md)를 전제로 한다. 제어기별 변주는 [31 EPOS-10i](31-EPOS-10i-OTA사양서.md) · [32 EPOS-30i §11](32-EPOS-30i-업데이트-사양서.md) · [33 ECU §4](33-ECU-OTA사양서.md). |
...
- **SGW** — OBD-II 외부 CAN을 격리하는 CAN 게이트웨이. HSM 보유. DMS License를 검증해 특정 CAN 선로를 개방한다.
...
2. **라이선스 발급** — Diagnostic License CA가 단수명 DMS License를 발급한다([10 §2.3](10-PKI-사양서.md), [20 §7](20-프로비저닝-사양서.md)).
...
| 계층 | 검증 주체 | 대상 | 무엇을 막나 |
|------|-----------|------|-------------|
| **게이트웨이 접근제어** | SGW(UDS `0x29`) / TGU(mTLS) | DMS License(PKI·Role) | 인가되지 않은 도구의 **내부 망 진입** |
...
- **"같은 라이선스로 두 번"이 아니다:** DMS License는 게이트웨이(SGW/TGU)에서만 검증한다. EPOS-10i는 암호 능력이 없어 라이선스(PKI)를 검증하지 못하며, `0x27` 세션 게이트는 seed-key라 라이선스와 무관하다.
- **용어:** "이중 검증"은 *하나의 DMS License를 SGW와 TGU 두 게이트웨이가 각자 방식으로 검증*하는 것(경로 분기)이다. §3의 `0x29`→`0x27`은 *한 경로에서 게이트 통과 후 대상 세션을 여는 직렬 단계*다.
...
| 3 | DMS→SGW | `29 01 …` | verifyCertificateUnidirectional: `commConfig(0x00)` + `lenCertClient(2B)` + **DMS License(DER)** + `lenChallengeClient(2B)` + challengeClient |
...
- SGW는 4단계에서 `Root → Diagnostic License CA → DMS License` 체인을 HSM으로 검증하고 Role을 추출한다.
...
- ECU(엔진 제어기) 유선의 진단전용 Private CAN 라우팅(수정 불가 진단툴을 CAN ID 필터링으로 통제)은 이 선로 격리·개방 메커니즘의 특화 형태다 → [33 §4.2](33-ECU-OTA사양서.md).

---

## 7. TGU 접근제어 (이더넷)

TGU는 DoIP를 쓰지 않으므로, DMS License 인증서를 클라이언트 인증서로 하는 **mTLS**로 인증하고, Role에 따라 이더넷 경로를 개방한다(DoIP의 routing activation 게이트키핑을 TGU가 대체). 같은 DMS License를 SGW는 `0x29`로, TGU는 mTLS로 검증한다.
...
DMS License의 Role OID([10 §2.3](10-PKI-사양서.md))가 허용 범위를 정한다. 게이트웨이가 인증 성공 후 이 매핑으로 개방 범위를 제한한다.
...
- **소프트웨어 진위를 판정하는 것은 CS Cert 하나다.** 다운로드 채널의 서버 인증서는 접속 상대가 정품 서버임만 증명한다. 내려받은 Component가 정품인지는 증명하지 못한다. 진위는 Component에 동봉된 CS Cert로 판정하므로(§4), 다운로드 채널과 무관하게 오프라인 현장에서도 검증할 수 있다. 두 인증서의 구분은 [10 §2.2](10-PKI-사양서.md).
