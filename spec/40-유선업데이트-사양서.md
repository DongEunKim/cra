...
- **비중복 근거:** 게이트웨이 계층은 "누가 내부 망에 들어오는가"(경계)를, 대상 세션 계층은 "이 ECU를 재프로그래밍해도 되는가"(엔드포인트)를 지킨다. 내부 CAN은 SecOC 미적용이라 스푸핑·재생 잔여위험이 있으므로, 게이트를 통과한 뒤에도 대상 자체 세션 게이트가 독립적으로 필요하다(defense-in-depth). 하나만 두면 — 게이트만: 내부 스푸핑 시 임의 노드가 대상 플래시 / 세션만: 외부 도구가 내부 망 전체에 무제한 진입.
