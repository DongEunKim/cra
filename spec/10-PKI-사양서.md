...
| **상태** | Draft |
| **관계** | [20 프로비저닝 사양서](20-프로비저닝-사양서.md) · [30 OTA 시스템 사양서](30-OTA-시스템사양서.md)와 함께 읽는다. |
...
- 각 디바이스는 **Root CA 공개키만** 신뢰앵커로 보유한다. 공장에서 주입한다([20 §1](20-프로비저닝-사양서.md)).
- 중간 CA·리프 인증서는 앵커가 아니라 검증 대상이며, 검증 시점에 제공된다.
...
3. 유효기간(Not Before/After)을 확인한다(§6 신뢰 시각 필요).
...
- **오프라인 경계(TGU → EPOS-10i, CAN):** EPOS-10i는 폐기를 확인할 수 없다. 이 구간의 무결성은 TGU가 대행 검증하며([30 §4.2](30-OTA-시스템사양서.md)), CS Cert는 단수명 회전으로 폐기 노출을 줄인다.

### 5.3 갱신·교체·회전

- **디바이스 인증서:** 만료 전 갱신한다. 폐기 시 새 인증서를 발급해 캠페인으로 내려보내고, 검증 성공 후 교체한다(실패 시 이전 인증서로 복귀).
- **CS Cert:** Code Sign CA가 새 리프를 발급해 회전한다. 신뢰앵커(Root)는 고정되므로 디바이스 재프로비저닝이 필요 없다.
