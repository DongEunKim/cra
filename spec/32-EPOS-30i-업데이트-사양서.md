...
- **준비 위치·주체:** 내부 계층(원본 바이너리 **암호화**(AES-128-CTR, §5) + 이미지 **ECDSA P-256 서명**(§4))은 **상류(GPDM 릴리스 단계)에서 중앙이 준비**한다. **개발자가 아니다.** 그 결과물 SE Binary(Signed-Encrypted Binary, 이미지 헤더 포함, §6)는 **GPDM의 Component 소스로 eSync에 전달**된다([30 §1.2](30-eSync-OTA-공통아키텍처.md)). 준비의 시점·주체·제조 배포 세부는 상류 인프라 과제로 본 사양 범위 밖이며, eSync는 이를 **통짜 페이로드**로 취급한다.
