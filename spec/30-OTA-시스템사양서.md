...
- 검증 신뢰앵커(Root CA 공개키)는 각 노드가 로컬 보유한다([10 §3.1](10-PKI-사양서.md)).
...
EPOS-10i는 서명을 검증하지 못하므로 **TGU의 eSync Agent가 대행 검증**한다. TGU는 Root CA 공개키를 신뢰앵커로 보유한다([20 §1.3](20-프로비저닝-사양서.md)).
...
1. ClientHello.
2. 서버 인증서 제시.
3. 클라이언트가 서버 인증서 검증(체인·유효기간·CN·OCSP).
4. 클라이언트 인증서 제시(CN=Serial).
5. 서버가 클라이언트 인증서 검증(체인·CN·OCSP).
6. 검증 성공 시 mTLS 수립.
