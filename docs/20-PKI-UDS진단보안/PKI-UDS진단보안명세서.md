...
| **적용 대상** | 통합 디바이스 CA, 제어기(TGU/SGW/ECU), 진단툴(DMS), 클라우드(VLM/AWS IoT) |
...
1. **단일 CA 체계:** 복잡한 장비/사용자 CA 구분을 없애고, OEM 산하의 단일 `통합 디바이스 CA`가 하드웨어 제어기(TGU, SGW)와 진단툴(DMS PC)의 인증서를 일괄 발급한다.
2. **논리적 바인딩:** 하드웨어 인증서에 장비 번호(VIN)를 포함하지 않으며, 클라우드(VLM) 레벨의 데이터베이스 매핑과 SGW의 로컬 텍스트(VIN) 주입으로 대체한다.
3. **오프라인 중심 보안:** SGW는 인터넷(VLM) 통신 없이 내장된 Root/CA 공개키를 활용하여 오프라인 상태에서 DMS의 접근 권한을 수학적으로 검증한다.
4. **내부망 암호화 유예:** 기존 J1939(Classic CAN) 통신 환경의 대역폭 한계를 고려하여, 하위 ECU 노드 간의 SecOC(대칭키 암호화)는 본 스펙에서 제외한다.

![그림 1 · 통합 디바이스 CA 기반 PKI 발급 아키텍처](../../assets/diagrams/pki-그림1-CA발급아키텍처.svg)

*[그림 1] 통합 디바이스 CA 기반 PKI 발급 아키텍처*
...
| **CA 공개키** | `.der` 또는 `.key` | SPKI 바이너리 구조: X.509 포맷이 아닌, CA의 순수 공개키 값만 추출한 DER 인코딩 | EOL 공정에서 SGW에 주입 (UDS 서명 검증용 잣대) |
...
통합 디바이스 CA에서 발급되는 인증서는 '확장 필드(Extensions)'를 통해 하드웨어와 라이선스를 구분한다.
...
1. **CA 공개키 (.der):** 타 디바이스(DMS)의 인증서 유효성을 수학적으로 검증하기 위한 잣대. (순수 공개키 데이터, SPKI)
2. **장비 식별자 (String):** 자기 자신이 어떤 장비인지 자각하기 위한 `텍스트 포맷의 VIN` (예: `"VIN-KOREA-0001"`)

> SGW는 자신을 외부로 증명할 필요가 없으므로 개인키·인증서를 주입하지 않으며, **타인을 검증하는 잣대(CA 공개키)와
...
| 5 | SGW (내부) | - | **[로컬 검증]** ① 내장 CA 공개키로 DMS 인증서 위변조·만료일 확인 ② DMS 공개키 추출 후 난수 서명값 일치 확인 ③ 인증서 내 Custom OID(Role) 확인. |
...
OEM 통합 디바이스 CA 체계에 속하지 않는 외자품 ECU와 상호작용하기 위해 주체별로 역할을 분리한다.
...
- **SGW 로컬 검증 앵커화:** EOL 테스터는 SGW에 향후 UDS 통신 검증을 위한 **'OEM Root CA 공개키(.der)'**와 자신이 속한 장비 인식을 위한 **'문자열 VIN 텍스트'**만 주입한다. (§5)
...
- **SGW 로컬 방화벽 통과:** SGW는 내장 Root CA 공개키로 DMS 서명과 Role을 오프라인 상태에서 즉각 수학적으로 검증한 후, 조건 부합 시 UDS 0x29 진단 세션을 개방한다. 외자품 ECU에 대해서는 DMS 내부의 벤더 제공 라이브러리를 호출하여 추가 인증(0x27 등)을 수행한다. (§7.2)
