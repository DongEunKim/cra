...
>
> **근거(생애주기·CRA):** 제어기 수명이 20년(DOC-20 §4.1)에 달해 그 사이 CS-IC는 만료·교체된다. Root만 고정
...
> 회전 유연성은 포기한다. (실시간 인증서 폐기는 오프라인 타겟 특성상 별도 과제 → [REF-03](../참조-CRA배경/REF-03-추적성-적합성-개요.md))
...
│   │   │    * FW를 암호화한 대칭키(AES-TSK)를 대칭키 'SW-Group-Key'로 랩핑함
│   │   └── 🔗 설치 패키지 해시
│   └── 🔐 배포 서명 (Distribution Signature)
│
└── 📂 [2] 설치 패키지 (Installation Package)
...
- **검증 절차:**
  1. 인스톨러로부터 `설치 패키지`를 수신하고, 암호화된 경우 내장 대칭키(SW-Group-Key)로 TSK를 풀어 펌웨어를 복호화한다.
...
## 4. 대칭키(SW-Group-Key) 기반 FW 암호화 (Secure Flash)

펌웨어의 기밀성을 보호하면서도 복잡한 비대칭 연산을 줄이고 대규모 배포 스케일링을 만족하기 위해,
**대칭키 기반의 'SW-Group-Key'** 구조를 도입한다.

![그림 2 · CSR 기반 SW-Group-Key 인증서 매립 주입 시퀀스](../../assets/diagrams/fota-그림2-대칭키주입시퀀스.svg)

*[그림 2] CSR 기반 대칭키(SW-Group-Key) 인증서 매립 주입 시퀀스*
...
2. **[OEM 영역] CSR 전송 및 KMS 연동:** 테스터는 VPN을 통해 CSR을 OEM의 **`Dev-IC`**로 전송한다. Dev-IC는 내부망에 격리된 **KMS**에 대칭키 **`SW-Group-Key`**를 요청하고, 이를 CSR 내의 `Dev-Pub`으로 비대칭 암호화 받는다.
3. **[OEM 영역] X.509 확장 필드 매립:** Dev-IC는 디바이스 인증서를 발급할 때 암호문을 **X.509 v3 Custom Extension** 영역에 매립하여 서명한 뒤 테스터로 반환한다.
4. **[부품사 영역] 로컬 인증서 파싱 및 복호화:** 테스터가 인증서를 제어기에 주입하면, 제어기는 확장 필드 안의 암호문을 자신의 `Dev-Priv`로 풀고 최종 대칭키 `SW-Group-Key`를 HSM에 영구 저장한다.

### 4.2 FOTA 배포 서버의 암호화 프로세스

1. **[펌웨어 암호화]:** 배포 서버는 일회용 임시 세션 키(`AES-TSK`)를 생성하여 펌웨어를 고속 암호화한다.
2. **[TSK 랩핑]:** 배포 서버는 펌웨어를 풀 열쇠인 `AES-TSK`를 대칭키 **`SW-Group-Key`**로 암호화(Wrapping)하여 배포 매니페스트에 동봉한다.
3. **[로컬 복호화]:** 타겟 제어기는 자신의 HSM에 내장된 `SW-Group-Key`로 `AES-TSK`를 복호화하고, 이를 이용해 최종 펌웨어를 복호화한다.
...
- [DOC-20 PKI·UDS 0x29 진단 보안 명세서](../20-PKI-UDS진단보안/README.md)
- [REF-03 추적성·적합성 개요](../참조-CRA배경/REF-03-추적성-적합성-개요.md) (구 DOC-30)
